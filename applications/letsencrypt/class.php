...
class ACME
{
	 ...
	 * @return bool 
	 */
	public static function order(LEClient $client, $common_name, $domains, $io, $autorenew = true, $verbose = false, $manual = false)
	{
		if (count($domains) == 0 || !is_array($domains)) throw new \PuzzleError("Domains cannot be empty!");
		if ($common_name == "") throw new \PuzzleError("Please specify common name using");
		...
		if (!$manual) {
			...
			$io->out("[INFO] No cloudflare connection, since manual mode is activated\n");
		}
			...
			if ($pending !== false) {
				...
				foreach ($pending as $k => $d) {
					...
					if (!$manual) {
						...
						$io->out("\n[INFO]\n");
						$io->out("Please add $subdomain with values {$d["DNSDigest"]} to your domain as TXT.\n");
						$io->out("Press ENTER when done...\n");
						$io->in();
					}
		...
		if ($order->allAuthorizationsValid()) {
			...
			if ($verbose) $io->out("[ERROR] Cannot continue. Uncompleted challenge has been found...\n");
			$pending = $order->getPendingAuthorizations(LEOrder::CHALLENGE_TYPE_DNS);
